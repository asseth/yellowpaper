...
\begin{abstract}
Le paradigme de la blockchain, lorsqu'il est couplé à un mécanisme de transaction sécurisé par la cryptographie, a prouvé son utilité au travers de projets tels que Bitcoin. Chacun de ces projets peut être vu comme une simple application basée sur une unique ressource de calcul décentralisée. Nous pouvons appeler ce paradigme une machine transactionnelle unique à état partagé.

De manière générale, Ethereum implémente ce paradigme. Ethereum fournit un grand nombre de telles ressources, chacune possédant un état distinct et un code fonctionnel, mais en plus capable d'intéragir entre elles au travers d'un framework d'envoi de messages. Nous discuterons de sa conception, des problèmes de conception, des opportunités que cela fournit et des futurs obstacles que nous envisageons.
\end{abstract}
