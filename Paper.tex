...
\begin{abstract}
Le paradigme de la blockchain, lorsqu'il est couplé à un mécanisme de transaction sécurisé par la cryptographie, a prouvé son utilité dans nombre de projets, Bitcoin n'étant pas le moindre. Chacun de ces projets peut être vu comme une simple application basée sur une ressource de calcul décentralisée mais unitaire. Nous pouvons appeler ce paradigme une machine transactionnelle unique à état partagé.

Ethereum implémente ce paradigme de façon généraliste. Ethereum fournit un grand nombre de telles ressources, chacune possédant un état distinct et un code fonctionnel, mais aussi susceptibles d'intéragir au travers d'un framework d'échange de messages. Nous discuterons de sa conception, des problèmes d'implémentation, des opportunités que cela fournit et des obstacles que nous anticipons.
\end{abstract}
...
Internet étant présent dans les plupart des endroits de la planète, la transmission de l'information globale est devenue extrèmement peu onéreuse. Les innovations technologiques tels que Bitcoin ont démontré, par le pouvoir d'un mécanisme de consensus et le respect volontaire du contrat social, qu'il était possible d'utiliser Internet pour créer un système décentralisé de transfert de valeurs, partagé à travers le monde et gratuit à utiliser. Ce système peut être décrit comme une version très spécifique d'une machine a état basée sur des transactions et sécurisée par la cryptographie. Les système de suivi tels que Namecoin ont adapté ce système d' ``application monétaire'' en d'autres application, aussi simples soient-elles.

Ethereum est un projet qui cherche à créer une technologie généralisée, c'est à dire un technologie sur laquelle tous les concepts fonctionnant sur une machine a état basée sur des transactions pourraient être construits. De plus, Ethereum a pour but de fournir au développeur final un système intégré bout en bout afin de  construire des logiciels basés sur un paradigme de calcul jusque-là inexploré par le grand public : un framework de calcul d'adressage d'objets de confiance.

\subsection{Facteurs déterminants} \label{ch:driving}

Ce projet a beaucoup de buts; un des buts principaux étant de faciliter les transactions entre deux individus consentants qui n'auraient aucun moyen autrement de se faire confiance l'un l'autre. Cela peut être dû à une séparation géographique, une difficulté d'intéraction, ou peut-être encore à l'incompatibilité, l'incompétence, la réticence, le coût, l'incertitude, la gêne ou encore la corruption des systèmes légaux existants. En définissant un système de changement d'état à travers un langage riche et non ambigu, en architecturant de plus un système tel que nous pouvons raisonablement attendre qu'un accord soit appliqué de manière autonome, nous pouvons fournir un moyen à cette fin.

Les opérations dans le système proposé pourraient avoir beaucoup d'attributs qui n'existent pas souvent dans le monde réel. L'incorruptibilité de jugement, souvent difficile à trouver, est naturelle lorsqu'elle vient d'un interpreter algorithmique désintéressé. La transparence, ou la capacité de voir exactement comment un état ou un jugement est arrivé à travers les logs d'une transaction ou les codes instructionnels, n'arrive jamais parfaitement dans les systèmes basés sur l'humain, notamment car le langage naturel est nécéssairement vague, que de l'information est souvent manquante et que les vieux préjudices sont difficiles à identifier.

De manière générale, je souhaite fournir un système dans lequel les utilisateurs peuvent être sûrs que peu importe avec quel autres individus, systèmes ou organisations ils interagissent, ils peuvent avoir une confiance absolue dans les possibles résultats et comment ces résultats sont déterminés.

\subsection{Previous Work} \label{ch:previous}

\cite{buterin2013ethereum} a proposé initalement le noyau de ce travail fin Novembre 2013. Bien qu'elle ait maintenant évolué sur différents points, la fonctionnalité clé d'une blockchain possédant un langage Turing-complet et d'une capacité illimitée de stockage inter-transaction fonctionnant efficacement demeure inchangée.

\cite{dwork92pricingvia} a fourni la premier travail d'usage de preuve cryptographique de dépense (``proof-of-work, ou preuve de travail'') ayant pour but de  de transmettre un signal de valeur à travaers Internet. Le signal de valeur était utilisé ici comme un mécanisme de dissuasion de spam plutôt que n'importe quelle sorte de monnaie, mais a démontré de manière critique le potentiel d'un chemin basique de donnée pour porter un \textit{fort signal économique}, autorisant un receveur à faire une vérification physique sans avoir à dépendre de la \textit{confiance}. \cite{back2002hashcash} a plus tard produit un système dans la même veine.

Le premier exemple d'utilisation de la preuve de travail en tant que signal économique fort pour sécuriser une monnaie a été utilisé par \cite{vishnumurthy03karma:a}. Dans cet exemple, le jeton a été utilisé pour surveiller l'échange de fichier en peer to peer, s'assurant que les ``consommateurs'' sont capcable de fire des micro-paiements aux ``fournisseurs'' pour leurs services. Le modèle de sécurité apporté par la preuve de travail a été augmentée avec les signatures digitales et un registre dans le but d'assurer le fait que l'historique des enregistrements ne puisse pas être corrompue, et que les acteurs malicieux ne puissent pas inventer des paiements ou se plaindre à propos du service délivré. Cinq ans plus tard, \cite{nakamoto2008bitcoin} a introduit un autre jeton de valeur basé sur une preuve de travail sécurisée, de plus large portée. Le fruit de ce projet, Bitcoin, est devenu  le premier registre global décentralisé de transactions largement adopté.

D'autres projets créés sur Bitcoin ont réussis; les altcoins ont introduit un grand nombre d'autres monnaies en modifiant le protocole. Quelques uns des plus connus sont Litecoin et Primecoin, cités par \cite{sprankel2013technical}. Les autres projets ont su prendre le coeur du contenu du mécanisme de valeur et de le reproposer; \cite{aron2012bitcoin} a parlé, par exemple, du projet Namecoin qui a pour but de fournir un système de résolution de nom décentralisé.

D'autres projets visent toujours à construire par dessus le réseau Bitcoin lui-même, élevant le gros volume de valeur placé dans le système et le gros volume de puissance de calcul destiné au mécanisme de consensus. Le projet Mastercoin, proposé initialement par \cite{mastercoin2013willett}, vise à construire un protocole plus riche, impliquantt beaucoup de fonctionnalités de haut niveau par dessus le protocole Bitcoin à travers l'utilisation d'un grand nombre de parties auxilières au protocole de base. Le projet Colored Coins, proposé par \cite{colouredcoins2012rosenfeld}, a une stratégie similaire mais plus simple, embellissant les règles d'une transaction afin de casser la fongibilité de la bonnaie de base Bitcoin et autoriser la création  et le traçage des jetons à travers un ``chroma-wallet'', une surcouche logicielle fonctionnant avec le protocole.

Des travaux supplémentaires ont été effecués avec pour idée d'écarter les fondations de la décentralisation; Ripple, décrit par \cite{boutellier2014pirates}, a cherché à créer un système fédéré pour l'échange de devises, en créant efficacement un nouveau système financier de liquidation. Cela a démontré que de gros gains d'efficience peuvent être faits si le principe de la décentralisation est rejeté.

Plus tôt un travail sur les smart contracts a été effectué par \cite{szabo1997formalizing} et \cite{miller1997future}. Autour des années 90 il est devenu clair qu'une mise en application algorithmique des accords pourrait devenir une force significative dans la coopération entre les humains. Bien qu'aucun système spécifique n'ait été proposé pour implémenter un tel système, il a été proposé que le futur du droit devrait être profondément atteint par ce genre de systèmes. Dans cette optique, Ethereum peut être vu comme une implémentation générale d'un tel système de \textit{droit cryptographique}.

%E language?

\section{Le paradigme de la blockchain} \label{ch:overview}

Ethereum, pris dans son ensemble, peut-être vue comme une machine a état basée sur des transactions: nous commençons avec un état génésis (de genêse), et exécutons de stransactions de manière incrémentale pour le transformer en un état final. c'est l'état final que nous acceptons comme la ``version'' canonique du monde d'Ethereum. l'état peut inclure des informations tels que les soldes de comptes, la réputation, des accords de confiance, la donnée portant sur l'information du monde physique; en résumé, tout ce qui peut déjà être représenté par un ordinateur est admissible. Les transactions représentant une passerelle valide entre deux états; la partie `valide' est importante---il existe beaucoup plus de changements d'état invalides que de changements d'état valides. Les changements d'état invalides pourraient, \eg être des choses telles que réduire le solde d'un compte sans remplir ailleurs un montant équivalent. Un changement d'état valide l'est au travers d'une transaction. Concrètement: 
\begin{equation}
...
Les transactions sont regroupées en blocs; les blocs sont chainés les un les autres en utilisant un hash cryptographique comme référence. Les blocs fonctionnent comme un journal, enregistrant ensemble une série de transactions avec le bloc précédant et un identifiant pour l'état final (l'état final n'est pas stocké lui-même---cela serait beaucoup trop volumineux). Ils ponctuent également la série de transaction avec une incitation pour les noeuds à \textit{miner}.

Miner est le processus de dédier de l'effort (travailler) pour soutenir une série de transactions (un bloc) contre n'importe quel autre bloc de compétiteur. Cela est mis en oeuvre grâce à une preuve cryptographique sécurisée. Ce schema est connu comme la preuve de travail (proof-of-work), et est étudié en détail dans la section \ref{ch:pow}.


Concrètement, en détail:
\begin{eqnarray}
...
Où $\Omega$ est la fonction de changement d'état finalisant le bloc (une fonction qui gagne une partie nommée); $B$ est ce bloc, qui inclut une série de transactions parmi d'autres composants; et $\Pi$ est la fonction de changement d'état du niveau du bloc.

C'est la base du paradigme de la blockchain, un moddèle qui forme la colonne vertébrale, non seulement d'Ethereum, mais également de tous les systèmes de consensus décentralisés basés sur des transactions à l'heure actuelle. 

\subsection{Valeur}


Dans le but d'inciter le calcul au travers du réseau, il y a besoin d'une méthode convenue pour transmettre de la valeur. pour régler ce problème, Ethereum a une monnaie intrinsèque, l'Ether, aussi connue comme {\small ETH}, et parfois en référence à l'Anglais Ancien \DH{}. La plus petite sous-dénomination de l'Ether, et celle dans laquelle toutes les valeurs entières de la monnaie sont comptées est le Wei. Un Ether représente $10^{18}$ Wei. il existe d'autres sous-dénominations de l'Ether:
\par
\begin{center}
\begin{tabular}{rl}
\toprule
...
Au travers du présent document, chaque référence à la valeur, dans le contexte de l'Ether, comme un solde de compte ou un paiement, sera exprimé en Wei.

\subsection{Quelle histoire?}

Alors que le système est décentralisé et que chaque parties a une opportunité de créer un nouveau bloc sur un ancien bloc pré-existant, la structure résultante est nécéssairement un arbre de blocs. De telle sorte à former un consensus sur quel chemin de la racine (le bloc genesis) à la feuille (le bloc contenant les transactions les plus récentes) à travers cette structure d'arbre, connue sous le nom de blockchain, il doit y avoir un schéma d'accord. Si jamais il y avait un désaccord entre les noeuds sur quel chemin en bas de l'arbre des blocs est la `meilleure' blockchain, alors un fork est effectué.  

Cela voudrait dire que passé un point donné dans le temps (bloc), plusieurs états du système peuvent coexister: certains noeuds croyant qu'un bloc contient les transactions canoniques, d'autres noeuds croyant un autre bloc être canonique, contenant potentiellement des transactions radicalement différentes ou incompatibles. Ceci est à éviter à tous coûts, comme l'incertitude qui pourrait résulter et qui pourrait tuer la confiance dans le système entier. 
...
Les deux ensembles de haut niveau, hautement structurés, les valeurs d'état, sont notés en lettres Grecques en petits caractères gras. Ceux catégorisant l'état mondial sont notés $\boldsymbol{\sigma}$ (ou une variante là dessus) et ceux de l'état de la machine, $\boldsymbol{\mu}$.

Les fonctions opérant sur ces valeurs hautement structurées sont notés en lettre Grecque majuscule, \eg $\Upsilon$, la fonction de transition d'état d'Ethereum.

Pour la plupart des fonctions, une lettre majuscule est utilisée, par exemple $C$, la fonction de coût général. Elles peuvent être subscriptées ( TODO: écrit en petit en bas à droite?) pour indiquer des variantes spécialisées, \eg $C_\text{\tiny SSTORE}$,
la fonction de coût pour l'opération {\tiny SSTORE}. Pour les fonctions spécialisées et possiblement définies de manière externe, je peux formater comme un texte de machine à écrire, \eg la fonction de hachage Keccak-256 (selon l'entrée gagnante pour le concours SHA-3) est dénotée $\texttt{KEC}$ (et fait généralement référence à une simple Keccak). Aussi, $\texttt{KEC512}$ fait référence à une fonction de hachage Keccak 512.

Les tuples sont typiquement notés en lettres majuscules, \eg $T$, est utilisé pour noter une transaction Ethereum. Ce symbole peut, si il est définit en conséquence, être subscripté (TODO: écrit en petit en bas à droite) pour faire référence à un composant individuel, \eg $T_n$, dénote le nonce de ladite transaction. la forme du sucscript (TODO) est utiliser pour noter sa taille; \eg des subscripts (TODO) en majuscule font référence a des scripts avec des composants subscriptables (TODO).

Les scalaires et les séquences d'octets de taille fixe (synonyme, les tableaux), sont notés avec une lettre normale minuscule, \eg $n$ est utilisée dans le document pour représenter un nonce de transaction. Ceux qui ont particulièrement une signification spéciale peuvent être grecques, \eg $\delta$, le nombre d'éléments requis sur la pile pour une opération donnée;

Des séquences de longueur arbitraires sont typiquement notés en lettre minuscules et en gras, \eg $\mathbf{o}$ est utilisé pour représenter la séquence d'octets donnée en tant que donnée de sortie d'un appel de message. Pour les valeurs particulièrement importantes, une lettre en majuscule et en gras peut-être utilisée.

Tout au long du document nous supposons que les valeurs scalaires sont des entiers positifs, et qu'elles appartiennent a l'ensemble $\mathbb{P}$. L'ensemble de toutes les séquences d'octets est $\mathbb{B}$, formellement défini dans l'annexe \ref{app:rlp}. Si une telle séquence d'ensembles est restreinte a un ensemble (? TODO) d'une longueur particulière, cela est noté avec un subscript (TODO), ainsi, l'ensemble de toutes les séquences d'octets de longueur $32$  est nommé $\mathbb{B}_{32}$ et l'ensemble de tous les entiers positifs plus petits que $2^{256}$ est nommé $\mathbb{P}_{256}$. Cela est décrit précisément dans la section \ref{ch:block}.

Les crochets sont utilisés pour indexer et référencer les composants individuels ou les sous-séquences de séquences, \eg $\boldsymbol{\mu}_\mathbf{s}[0]$ représente le premier objet sur la pile de la machine. Pour les sous-séqueces, les ellipses (? TODO) sont utilisées pour spécifier la tranche attendue, pour inclure les éléments de chaque limite, \eg $\boldsymbol{\mu}_\mathbf{m}[0..31]$ représente les 32 premiers objets de la mémoire de la machine.
...
En considérant les variantes des valeurs existantes, je suis la règle qu'au sein d'un périmètre donné pour définition, si nous supposons que la valeur d'`input' non modifiée soit représentée par l'espace réservé (? TODO) $\Box$ alors la valeur modifiée et utilisable est représentée comme $\Box'$, et les valeurs intermédiaires seraient $\Box^*$, $\Box^{**}$ \&c. Lors d'occasions vraiment particulières, dans le but de maximiser la lisibilité et seulement si non ambigu dans le sens, je peux utiliser des subscripts (? TODO) alphanumériques pour représenter des valeurs intermédiaires, particulièrement celles de notes particulières.		 

En considérant l'utilisation de fonctions existantes, prenant une fonction $f$, la fonction $f^*$ représente une version similaire element-wise(TODO), de la fonction de mapping plutôt qu'entre les séquences. Cela est précisément décrit dans la section \ref{ch:block}.

Je définit un nombre de fonctions utiles à travers ce document. Une des plus communes est $\ell$, qui évalue le dernier objet de la séquence donnée:
...
\subsection{L'état mondial} \label{ch:state}

L'état mondial (\textit{l'état}), est un mapping entre les adresses (identifiants de 160-bit) et l'état des comptes (une structure de données serialisée comme RLP, voir annexe \ref{app:rlp}). Bien que pas stockée sur la blockchain, on suppose que l'implémentation va maintenir ce mapping dans un arbre Merkle Patricia (\textit{trie}, voir annexe \ref{app:trie}). Le (TODO) trie nécessite une base de donnée en arrière plan qui maintient un mapping de tableau de bytes à tableau de bytes; nous nommons cette base de donnée sous jacente la base de donnée d'état. Elle a un grand nombre d'avantages; premièrement le noeud racine de cette structure est cryptographiquement dépendant de toute la donnée interne et son hash peut être utilisé comme une identité sécurisée pour l'état du système entier. Deuxièmement, étant une structure de donnée immutable, elle autorise n'importe quel état précédent (dont le hash racine est connu) à être appelé à nouveau (TODO : vérifier) en altérant simplement le hash racine accordingly (TODO). Comme nous stockons de tel hash racines dans la blockchain, nous sommes capable de revenir aux anciens états de manière triviale.

L'état d'un compte comprend les quatre champs suivants:

\begin{description}
\item[nonce] Une valeur scalaire égale au nombre de transactions envoyées de cette adresse, ou, dans le cas de compte contenant du code, le nombre de création de contrats faits par ce compte. pour les comptes ou les adresses $a$ dans l'état $\boldsymbol{\sigma}$, cela devra être représenté par $\boldsymbol{\sigma}[a]_n$.
\item[balance] Une valeur scalaire égale au nombre de Wei possédés par cette adresse. noté $\boldsymbol{\sigma}[a]_b$.
\item[storageRoot] Un hash de 256-bit du noeud racine d'un arbre merkle Patricia qui encode le stockage des contenus du compte (un mapping entre des valeurs entières de 256 bits), encodés dans le (TODO) tries comme un mapping entre le hash Keccak 256 bits des clés entières de 256 bits et les valeurs entières encodées par RPL de 256 bits. le hash est noté $\boldsymbol{\sigma}[a]_s$.
\item[codeHash] Le hash du code EVM de ce compte---c'est le code qui est exécuté lorsque cette adresse reçoit un appel de message; c'est immutable et ainsi, contrairement à tous les autres champs, ne peut pas être changé après construction. Tous ces fragments de code sont contenus dans la base de donnée d'état sous leur hash correspondant pour les récupérer plus tard. Ce hash est noté $\boldsymbol{\sigma}[a]_c$, et ainsi le code peut petre représenté par $\mathbf{b}$, sachant que $\texttt{\small KEC}(\mathbf{b}) = \boldsymbol{\sigma}[a]_c$.
\end{description}

Je souhaite typiquement faire référence, non pas hash racine du trie (TODO) mais à l'ensemble sous-jacent de paires de clés/valeurs stockée à l'intérieur, je définis une équivalence convenable: 
\begin{equation}
\texttt{\small TRIE}\big(L_I^*(\boldsymbol{\sigma}[a]_\mathbf{s})\big) \equiv \boldsymbol{\sigma}[a]_s
\end{equation}

la fonction d'effondrement (TODO) pour l'ensemble de paires de clé/valeur dans le trie (TODO), $L_I^*$, est définie comme la transformation element-wise (TODO) de la fonction de base $L_I$, définie par:
\begin{equation}
...
Il doit être compris que $\boldsymbol{\sigma}[a]_\mathbf{s}$ n'est pas un membre `physique' du compte et ne contribue pas a sa serialisation ultérieure.

Si le champ \textbf{codeHash} est le hash Keccak-256 de la string vide, par exemple $\boldsymbol{\sigma}[a]_c = \texttt{\small KEC}\big(()\big)$, alors le noeud représente un simple compte, parfois appelé comme un compte ``non-contrat''.

Ainsi nous devons définir une fonction d'effondrement de l'état du monde $L_S$:
\begin{equation}
...
cette fonction , $L_S$, est utilisée tout au long de la fonction de trie (TODO: trie) pour fournir une identité courte (hash) à l'état du monde. Nous supposons:
\begin{equation}
...
\end{equation}
Le protocole a été mis à jour à ce bloc, donc ce symbole apparait dans quelques équations de comptes, pour les changements.

\subsection{La Transaction} \label{ch:transaction}

Une transaction ($T$) est une instruction de signature cryptographique unique construite par un acteur externe du spectre d'Ethereum. Alors que nous supposons que l'ultime acteur externe sera humain par nature, les outils logiciels qui seront utilisé pour sa construction et sa propagation\footnote{Particulièrement, de tels `outils' pourraient en fin de compte devenir causalement écartées de leur création humaine---ou les humains peuvent devenir causalement neutres---de telle manière qu'il pourraient se comporter comme des agents autonomes. \eg Les contrats peuvent offrir des primes aux humains pour avoir envoyé des transactions pour initier leur exécution.}. Il y a deux types de transactions: celles qui entraînent des appels de message et celles qui entraînent la création de nouveaux comptes auxquels est associé du code (connu de manière informelle sous le nom de `création de contrat'). Chacun de ces types de transaction spécifient un nombre de champs communs:
...
\item[gasLimit] Une valeur scalaire égale au montant maximum de gas qui peut re utilisé en exécutant cette transaction. Cela est payé en amont, avant que tout calcul soit fait et ne peut pas être augmenté plus tard; $T_g$.
...
\item[value] Une valeur scalaire égale au nombre de Wei à transférer au destnataire de l'appel de message ou, dans le cas d'une création de contrat, comme une dotation pour le nouveau compte créé; $T_v$.
...
\begin{description}
\item[init] Un tableau d'octets de taille infinie contenant le code EVM pour la procédure d'initialisation du compte, $T_\mathbf{i}$.
\end{description}
...
Ici, nous supposons que tous les composants sont interprétés par le RPL comme des valeurs entières, avec l'exception d'octets de longueur arbitraire $T_\mathbf{i}$ et $T_\mathbf{d}$.
...
Le hash de l'adresse $T_\mathbf{t}$ est légèrement différent: c'est soit le hash d'une adresse de 20 octets, ou, dans le cas d'une création de contrat (et ainsi formellement égal à $\varnothing$), c'est la série RPL d'octets vides et ainsi le membre de $\mathbb{B}_0$:
