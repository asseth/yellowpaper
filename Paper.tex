...
&&&& Tous les calculs intermédiaires de cette opération ne sont pas modulo $2^{256}$. \\
\midrule
...
Si la sortie de cet algorithme se trouve sous l'objectif désiré, le nonce est valide. On note que l'application supplémentaire de \texttt{KEC} à la fin assure qu'il existe un nonce intermédiaire qui peut être fourni pour prouver qu'au moins une petite quantité de travail a été effectuée\,; cette rapide vérification externe de preuve de travail peut être employée contre les DDOS. Elle sert également à fournir une assurance statistique que le résultat est un nombre sur 256 bits non faussé.
