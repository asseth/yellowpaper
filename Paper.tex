...
L'évaluation de $\boldsymbol{\sigma}_P$ à partir de $\boldsymbol{\sigma}_0$ dépend du type de la transaction, création de contrat ou appel de message\,; nous définissons le n-uplet\,: état provisionnel de post-exécution $\boldsymbol{\sigma}_P$, gaz restant $g'$ et sous-état $A$\,:
\begin{equation}
...
On remarque que nous devons omettre la quatrième valeur du n-uplet renvoyé par $X$ pour évaluer correctement $\Xi$, d'où la notation $X_{0,1,2,4}$.
...
\begin{enumerate}
\item Valider (ou, pour du minage, déterminer) les \textit{oncles}\,;
